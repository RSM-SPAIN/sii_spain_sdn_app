...
function siiOnSaveRecord() {
    ...
    switch (rectype) {
        case 'customer': case 'vendor':
            ...
            break;
        case 'vendorbill': case 'vendorcredit': case 'invoice': case 'creditmemo': case 'creditcardcharge': case 'creditcardrefund': case 'cashrefund': case 'cashsale':
            ...
            function handleResponse(response) {
                ...
                if (!!response.getCode().toString().match(/^[2]/g)) {
                    ...
                    if ((response.getError().message || '').indexOf('The element type "link" must be terminated by the matching end-tag') > -1) {
                        rsm.createAnnouncer('custpage_x_siiec_banner', 'Error', 'Se han detectado errores en el envío. Revise que la subsidiria tiene certificado asignado y es váido. Contacte la pestaña de log para más detalles.', 'error');
                    } else {
                        rsm.createAnnouncer('custpage_x_siiec_banner', 'Error', 'Se han detectado errores en el envío. Contacte la pestaña de log para más detalles.', 'error');
                    }
